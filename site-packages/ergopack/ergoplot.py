...
from matplotlib import cm, patches, rcParams
    ...
    if eigVecBackwardFile is not None:
        ...
        if (eigVecForwardFile is not None) and makeBiorthonormal:
            ...
            if eigVecForwardFile is None:
                print('File for reading forward eigenvectors should be given'
                      'to contruct biorthonormal basis.')
            else:
...
def readSpectrumGenerator(nev, N, eigValForwardFile, eigVecForwardFile,
    ...
    """Read transfer operator spectrum from file and create a
    bi-orthonormal basis of eigenvectors and adjoint eigenvectors"""
...
def getSpectralWeights(f, g, eigVecForward, eigVecBackward, statDist=None,
    ...
    If the latter is not given, then the forward eigenvectors
    are assumed to cary it."""
    nev, N = eigVecForward.shape
    # Get stationary distribution
    if statDist is None:
...
def plotEigVecPolar(Xt, Yt, vectOrig, mask=None, xlabel=r'$x$', ylabel=r'y',
    ...
    if combine:
        ...
        cs = axPhase.contour(
            X, Y, amp2d, colors='k', linestyles='-', linewidths=linewidths)
...
def plotEigVecPolarCombine(
        Xt, Yt, vectOrig, mask=None, xlabel=r'$x$', ylabel=r'y',
        csfmt='%1.1f', alpha=0., cmap=cm.RdBu_r, combine=True,
        ampMax=None, ampMin=None, nlevAmp=11, csfilter=1, xmin=None,
        xmax=None, ymin=None, ymax=None, linewidths=2,
        xtick_formatter=None, ytick_formatter=None,
        cbar_format=None):
    fig = plt.figure()
    ax = fig.add_subplot(111)

    # Set axis ticks format
    if xtick_formatter is not None:
    ...
    if mask is None:
        ...
        phase2d = np.zeros((N,))
        phase2d[mask2] = phase
        phase2d = phase2d.reshape(X.shape)
        phase2d = np.ma.masked_array(phase2d, mask=~mask2.reshape(X.shape)).T
    levels = np.linspace(-np.pi, np.pi, 5)
    p = ax.contour(X, Y, phase2d, levels, colors='k',
                   linestyles='-', linewidths=linewidths)
    for k in np.arange(10):
        filterContours(p, threshold=csfilter)
    # ax.clabel(p, inline=1, fontsize=fs_clabel, fmt=csfmt)
    ...
    if mask is None:
        ...
        amp2d = np.zeros((N,))
        amp2d[mask2] = amp
        amp2d = amp2d.reshape(X.shape)
        amp2d = np.ma.masked_array(amp2d, mask=~mask2.reshape(X.shape)).T
    levelsAmp = np.linspace(ampMin, ampMax, nlevAmp)
    cs = ax.contourf(X, Y, amp2d, levelsAmp, cmap=cmap,
                     vmax=ampMax, vmin=ampMin)
    cbar = plt.colorbar(cs)
    cbar.ax.set_yticklabels([cbar_format.format(float(yt.get_text()))
                             for yt in cbar.ax.get_yticklabels()])
    plt.setp(cbar.ax.get_yticklabels(), fontsize='large')
...
def plotRecCorrelation(lags, corrSample, corrRec, corrAna=None,
    ...
    if corrAna is not None:
        ax.plot(lags, corrAna, linestyle='-', color=colors[1], linewidth=lw)
    ax.plot(lags, corrRec, linestyle='--', color=colors[0], linewidth=lw)
    nyticks = 10
    ax.set_yticks(np.arange(ylim[0], ylim[1] * 1.01,
                            (ylim[1] - ylim[0]) / nyticks))
    ax.set_xlim(xlim)
    ax.set_ylim(ylim)
    ax.set_xlabel(xlabel, fontsize=fs_latex)
    ax.set_ylabel(ylabel, fontsize=fs_latex)
...
def plotEig(eigValGen, eigAna=None, markersize=None, condition=None,
            ...
            xticks=None, yticks=None, marker='o', markercolor='b',
            fig_ax=None):
    if condition is None:
        condition = markercolor

    # Create axis for eigenvalue and power spectrum panels
    fig, axEig = plt.subplots() if fig_ax is None else fig_ax
...
def plotEigPowerRec(
        angFreq, eigValGen, powerSample, powerRec, powerAna=None, eigAna=None,
        markersize=None, condition=None, xlabel=None, ylabel=None, zlabel=None,
        xlim=None, ylim=None, zlim=None, xticks=None, yticks=None,
        zticks=None):
    # Create axis for eigenvalue and power spectrum panels
    ...
    # Plot eigenvalues
    markersize = markersize if markersize is None else 20.
    if eigAna is not None:
        axEig.scatter(eigAna.real, eigAna.imag, c=colors[1],
                      s=int(markersize*2.), marker='x')
    axEig.scatter(eigValGen.real, eigValGen.imag,
                  s=int(markersize*8.), marker='+', c=colors[0],
                  cmap=cm.Greys_r)
    ...
    # Plot power spectra
    axPow.plot(powerSample, angFreq, linestyle='-', color='k')
    if powerAna is not None:
        axPow.plot(powerAna, angFreq, linestyle='-', color=colors[1])
    axPow.plot(powerRec, angFreq, linestyle='--', color=colors[0], linewidth=2)
...
def plotEigPowerRec3D(angFreq, eigValGen, weights, powerSample, powerRec,
    ...
    for k in np.arange(eigValPlot.shape[0]):
        ...
        ax.plot([0., 0.],
                [eigValPlot[k].imag, eigValPlot[k].imag],
                [zlim[0],
                 powerSamplePlot[np.argmin(
                     np.abs(angFreq - eigValPlot[k].imag))]], '--b')
    if np.sum(~iFirst) > 1:
        ...
        ax.plot([0, 0.], [eigValGen[ieigVal][~iFirst][1].imag,
                          eigValGen[ieigVal][~iFirst][1].imag],
                [zlim[0], powerSamplePlot[
                    np.argmin(np.abs(angFreq - eigValGen[ieigVal][
                        ~iFirst][1].imag))]], '--g')
...
def getEigenCondition(eigVecForward, eigVecBackward, density=None,
    ...
    for ev in np.arange(nev):
        if forwardLebesgue:
            ...
            normForward = np.sqrt(
                np.sum(eigVecForward[ev] * densitym1
...
def pcolor_rectangle(x, y, data, dx=None, dy=None,
    ...
    if dy is None:
        dy = y[1] - y[0]
    if cmap is None:
        cmap = cm.get_cmap(plt.rcParams['image.cmap'], ncolors)
    N = x.shape[0]
    patch = []
    for k in range(N):
