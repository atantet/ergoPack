...
def getSpectralWeights(f, g, eigVecForward, eigVecBackward,
                       statDist, skipMean=False, forwardLebesgue=False):
...
and adjoint eigenvectors w.r.tw the stationary distribution."""
    ...
    for k in np.arange(ne):
        ...
        if forwardLebesgue:
            weights[k] *= (eigVecForward[:, k] * np.conjugate(ga)).sum()
        else:
            weights[k] *= (eigVecForward[:, k] * statDist * np.conjugate(ga)).sum()
...
def getEigenCondition(eigVecForward, eigVecBackward, density, forwardLebesgue=False):
