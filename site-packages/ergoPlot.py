...
def readSpectrum(eigValForwardFile, eigValBackwardFile=None,
                 ...
                 makeBiorthonormal=False, fileFormat="txt"):
    ...
    if fileFormat == 'bin':
        ...
        eigValForward = np.loadtxt(eigValForwardFile, complex)
    ...
    if eigValBackwardFile is not None:
        ...
        if fileFormat == 'bin':
            ...
            eigValBackward = np.loadtxt(eigValBackwardFile, complex)
    ...
    if eigVecForwardFile is not None:
        if fileFormat == 'bin':
            ...
            eigVecForward = np.loadtxt(eigVecForwardFile, complex)
            eigVecForward = eigVecForward[:, 0] + 1j*eigVecForward[:, 1]
        eigVecForward = eigVecForward.reshape(nev, -1)
        N = eigVecForward.shape[0]
        # Sort
        if makeBiorthonormal:
    ...
    if eigVecBackwardFile is not None:
        if fileFormat == 'bin':
            eigVecBackward = np.fromfile(eigVecBackwardFile, complex).reshape(nev, -1)
        else:
            eigVecBackward = np.loadtxt(eigVecBackwardFile, complex).reshape(nev, -1)
            eigVecBackward = eigVecBackward[:, 0] + 1j*eigVecBackward[:, 1]
        eigVecBackward = eigVecBackward.reshape(nev, -1)
        N = eigVecBackward.shape[1]
        # Sort
        if makeBiorthonormal:
