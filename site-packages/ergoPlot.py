...
def plotEigVecPolar(X, Y, vectOrig, mask=None, xlabel=r'$x$', ylabel=r'y',
                    alpha=0., cmap=cm.RdBu_r, combine=True):
    ...
    h = ax.contourf(X, Y, phase, levelsPhase, cmap=cmap)
    ax.set_xlim(X[0].min(), X[0].max())
    ax.set_ylim(Y[:, 0].min(), Y[:, 0].max())
    cbar = plt.colorbar(h)
    plt.setp(cbar.ax.get_yticklabels(), fontsize=fs_yticklabels)
    ...
    if combine:
        ax.contour(X, Y, amp, levelsAmp, linestyles='--', colors='k',
                   linewidths=2, vmin=-vmax, vmax=vmax)
    else:
        fig2 = plt.figure()
        ax2 = fig2.add_subplot(111)
        h = ax2.contourf(X, Y, amp, levelsAmp, cmap=cmap,
                         vmin=-vmax, vmax=vmax)
        ax2.set_xlim(X[0].min(), X[0].max())
        ax2.set_ylim(Y[:, 0].min(), Y[:, 0].max())
        cbar = plt.colorbar(h)
        plt.setp(cbar.ax.get_yticklabels(), fontsize=fs_yticklabels)
        ax2.set_xlabel(xlabel, fontsize=fs_latex)
        ax2.set_ylabel(ylabel, fontsize=fs_latex)
        plt.setp(cbar.ax.get_yticklabels(), fontsize=fs_yticklabels)
        plt.setp(ax2.get_xticklabels(), fontsize=fs_xticklabels)
