...
def readSpectrum(eigValForwardFile, eigValBackwardFile=None,
    ...
    if eigVecBackwardFile is not None:
        if fileFormat == 'bin':
            ...
            eigVecBackward = np.loadtxt(eigVecBackwardFile, complex)
...
def plot2D(X, Y, vectOrig, mask,
           xlabel=r'$x$', ylabel=r'y', alpha=0.):
    fig = plt.figure()
    ...
    coord = (X.flatten()[mask < N],
             Y.flatten()[mask < N])
    pcollection = pcolor_rectangle(coord[0], coord[1], vect,
                                   cmap=cm.RdBur_r, ncolors=levels,
                                   vmin=-vmax, vmax=vmax)
    ax.add_collection(pcollection)
    # Add colorbar
    cbar = map.colorbar(pcollection, size='3%')
...
def plot3D(X, Y, Z, vectOrig, mask, xlabel=r'$x$', ylabel=r'$y$', zlabel='$z$',
    ...
    coord = (X.flatten()[mask < N],
             Y.flatten()[mask < N],
             Z.flatten()[mask < N])
    scat = ax.scatter(coord[0], coord[1], coord[2], c=vect,
