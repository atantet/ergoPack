...
def plotEigVecPolar(Xt, Yt, vectOrig, mask=None, xlabel=r'$x$', ylabel=r'y',
    ...
    ampAlpha = amp[amp != 0]
    if alpha > 1.e-6:
        vmax = np.sort(ampAlpha)[int((1. - alpha) * ampAlpha.shape[0])-1]
        vmin = np.sort(ampAlpha)[int(alpha * ampAlpha.shape[0])-1]
        (amp[amp > vmax], amp[amp < vmin]) = (vmax, vmin)
    else:
        (vmax, vmin) = (np.max(amp), np.min(amp))
    ...
    if combine:
        if mask is None:
            axPhase.contour(X, Y, amp, levelsAmp, linestyles='--', colors='k',
                            linewidths=2, vmin=vmin, vmax=vmax)
        else:
            raise ValueError('Cannot combine phase and amplitude on same plot when using mask')
    else:
        figAmp = plt.figure()
