...
def readSpectrum(eigValForwardFile, eigValBackwardFile=None,
                 statDistFile=None,
                 eigVecForwardFile=None, eigVecBackwardFile=None,
                 makeBiorthonormal=False, nev=None, N=None):
    ...
    eigValForward = eigValForward[:nev]
    ...
    if eigValBackwardFile is not None:
        ...
        eigValBackward = np.loadtxt(eigValBackwardFile)
          # P = E^{-1} \Lambda E
        eigValBackward = eigValBackward[:, 0] + eigValBackward[:, 1]*1j
        eigValBackward = eigValBackward[:nev]
    ...
    if statDistFile is not None:
        statDist = np.loadtxt(statDistFile)
        ret += (statDist,)
    ...
    if eigVecForwardFile is not None:
        if N is None:
            ...
            eigVecForward = loadtxt_nrows(eigVecForwardFile, nev * N)
        # Make complex
        eigVecForward = (eigVecForward[:, 0] \
                         + eigVecForward[:, 1]*1j).reshape(N, nev)
        # Sort
        if makeBiorthonormal:
    ...
    if eigVecBackwardFile is not None:
        if N is None:
            ...
            N = eigVecBackward.shape[0] / nev
        else:
            eigVecBackward = loadtxt_nrows(eigVecBackwardFile, nev * N)
        # Make complex
        eigVecBackward = (eigVecBackward[:, 0] \
                          + eigVecBackward[:, 1]*1j).reshape(N, nev)
        # Sort
        if makeBiorthonormal:
        ...
        if makeBiorthonormal:
            if eigVecForwardFile is None:
                ...
                # If stationary distribution file was not given, assume uniform
                if statDistFile is None:
                    statDist = np.ones((N,), dtype=float) / N
...
def readSpectrumGenerator(nev, N, eigValForwardFile, eigVecForwardFile,
    ...
    isortBackward = np.empty((nev,), dtype=int)
    for ev in np.arange(nev):
        isortBackward[ev] = np.argmin(np.abs(eigValForward[ev] - np.conjugate(eigValBackward)))
    eigValBackward = eigValBackward[isortBackward]
    ...
    if readVectors:
        ...
        if makeBiorthonormal:
            ...
            for ev in np.arange(nev):
                ...
                eigVecBackward[:, ev] /= np.conjugate(norm)
        return (eigValForward, eigVecForward, eigValBackward, eigVecBackward, statDist)
    else:
...
def readSpectrumCompressed(eigValForwardFile, eigVecForwardFile,
                           eigValBackwardFile, eigVecBackwardFile,
                           statDist, readVectors=True, makeBiorthonormal=False):
    """Read transfer operator spectrum from file and create a bi-orthonormal basis \
    of eigenvectors and adjoint eigenvectors"""
...
If the latter is not given, then the forward eigenvectors are assumed to cary it."""
    ...
    for k in np.arange(ne):
        weights[k] = (f * statDist * np.conjugate(eigVecBackward[:, k])).sum()
        if forwardLebesgue:
            weights[k] *= (eigVecForward[:, k] * np.conjugate(g)).sum()
        else:
            weights[k] *= (eigVecForward[:, k] * statDist \
                           * np.conjugate(g)).sum()
            
    return weights

def spectralRecCorrelation(lags, f, g, eigValGen, weights, statDist,
                           skipMean=False, norm=False):
...
from the spectrum of the generator"""
    ...
    for k in np.arange(ne):
        components[k] = np.exp(eigValGen[k] * np.abs(lags)) * weights[k]
    ...
    if norm:
        cov = ((f - mean_f) * statDist \
               * np.conjugate(g - mean_g)).sum()
        reconstruction /= cov
        components /= cov
...
def spectralRecPower(angFreq, f, g, eigValGen, weights, norm=False,
                     statDist=None):
...
from the spectrum of the generator"""
    ...
    for k in np.arange(1, ne): 
        if np.abs(eigValGen[k].real) > 1.e-6:
            components[k] = -weights[k] / np.pi * eigValGen[k].real \
                            / ((angFreq - eigValGen[k].imag)**2 \
                               + eigValGen[k].real**2)
        else:
            components[k] = 0.
    reconstruction = components.sum(0).real
    ...
    if norm:
        if statDist is None:
            print 'The stationary density should be given when normalizing.'
        mean_f = (f * statDist).sum()
        mean_g = (statDist * np.conjugate(g)).sum()
        cov = ((f - mean_f) * statDist * np.conjugate(g - mean_g)).sum()
        reconstruction /= cov
        components /= cov
...
def plot3D(X, Y, Z, vectOrig, xlabel=r'$x$', ylabel=r'$y$', zlabel='$z$',
           alpha=0., scattersize=12, cmap=cm.RdBu_r):
    fig = plt.figure()
    ...
    if alpha > 1.e-6:
        ...
        vmax = np.max(np.abs(vect))
    scat = ax.scatter(X.flatten(), Y.flatten(), Z.flatten(), c=vect,
                      s=scattersize, linewidth=0, cmap=cmap,
                      vmin=-vmax, vmax=vmax)
    ax.set_xlim(X[:, 0, 0].min(), X[:, 0, 0].max())
    ax.set_ylim(Y[0, :, 0].min(), Y[0, :, 0].max())
    ax.set_zlim(Z[0, 0, :].min(), Z[0, 0, :].max())
    ax.set_xlabel(xlabel, fontsize=fs_latex)
    ax.set_ylabel(ylabel, fontsize=fs_latex)
...
def plotEigVecPolar(X, Y, vectOrig, xlabel=r'$x$', ylabel=r'y', alpha=0.):
    # Get amplitude and phase
    amp = np.abs(vectOrig).reshape(X.shape)
    phase = np.angle(vectOrig).reshape(X.shape)
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ampAlpha = amp[amp != 0]
    if alpha > 1.e-6:
        vmax = np.sort(np.abs(ampAlpha))[int((1. - 2*alpha) \
                                             * ampAlpha.shape[0])-1]
        amp[amp > vmax] = vmax
        amp[amp < -vmax] = -vmax
    else:
    ...
    h = ax.contourf(X, Y, phase, levelsPhase, cmap=cm.RdBu_r)
    ax.contour(X, Y, amp, levelsAmp, linestyles='--', colors='k', linewidths=2,
               vmin=-vmax, vmax=vmax)
    ax.set_xlim(X[0].min(), X[0].max())
    ax.set_ylim(Y[:, 0].min(), Y[:, 0].max())
    cbar = plt.colorbar(h)
...
def getEigenCondition(eigVecForward, eigVecBackward, density,
                      forwardLebesgue=False):
    ...
    for ev in np.arange(nev):
        if forwardLebesgue:
            densitym1 = np.ones((density.shape[0],))
            densitym1[density > 0] = density[density > 0]
            normForward = np.sqrt(np.sum(eigVecForward[:, ev] * densitym1\
