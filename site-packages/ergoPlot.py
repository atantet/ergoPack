...
def plot2D(X, Y, vectOrig, mask, xlabel=r'$x$', ylabel=r'y', alpha=0.,
           cmap=cm.RdBu_r, positive=False):
    fig = plt.figure()
    ...
    N = mask.shape[0]
    coord = (X.flatten()[mask < N], Y.flatten()[mask < N])
    (dx, dy) = (X[0, 1] - X[0, 0], Y[1, 0] - Y[0, 0])
    pcollection = pcolor_rectangle(coord[0], coord[1], vect, dx=dx, dy=dy,
                                   cmap=cmap, vmin=vmin, vmax=vmax)
    ax.add_collection(pcollection)
    # Add colorbar
    cbar = plt.colorbar(pcollection)
