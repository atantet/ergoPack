...
class recycle : public SolverManager<ScalarType,MV,OP> {
  ...
  /*! \brief Basic constructor for recycle.
   *
   ...
   */
  recycle(const Teuchos::RCP<Eigenproblem<ScalarType,MV,OP> > &problem,
  ...
  */
  ...
  // Constructor
template<class ScalarType, class MV, class OP>
recycle<ScalarType,MV,OP>::recycle(const Teuchos::RCP<Eigenproblem<ScalarType,MV,OP> >
				   &problem,
				   Teuchos::ParameterList &pl) :
  _problem(problem),
  _whch("LM"),
  ...
  ,_timerSolve(Teuchos::TimeMonitor::getNewTimer("Anasazi: recycle::solve()")),
  _timerRestarting(Teuchos::TimeMonitor::getNewTimer("Anasazi: recycle restarting"))
#endif
{
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(_blockSize <= 0, std::invalid_argument,
                     "Anasazi::recycle: \"Block Size\" must be strictly positive.");
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(_numBlocks <= _nevBlocks, std::invalid_argument,
                     "Anasazi::recycle: \"Num Blocks\" must be strictly positive\
 and large enough to compute the requested eigenvalues.");

  TEUCHOS_TEST_FOR_EXCEPTION(static_cast<ptrdiff_t>(_numBlocks)*_blockSize >
			     MVT::GetGlobalLength(*_problem->getInitVec()),
			     std::invalid_argument,
			     "Anasazi::recycle: Potentially impossible orthogonality requests.\
 Reduce basis size.");

  // step size: the default is _maxRestarts*_numBlocks,
  // so that Ritz values are only computed every restart.
  if (_maxRestarts) {
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(_stepSize < 1, std::invalid_argument,
			     "Anasazi::recycle: \"Step Size\" must be strictly positive.");

  // get the sort manager
  if (pl.isParameter("Sort Manager")) {
    _sort = Teuchos::getParameter<Teuchos::RCP<Anasazi::SortManager<MagnitudeType> > >
      (pl,"Sort Manager");
  }
  else {
    // which values to solve for
    _whch = pl.get("Which",_whch);
    TEUCHOS_TEST_FOR_EXCEPTION(_whch != "SM" && _whch != "LM" && _whch != "SR"
			       && _whch != "LR" && _whch != "SI" && _whch != "LI",
...
template<class ScalarType, class MV, class OP>
ReturnType
recycle<ScalarType,MV,OP>::solve() {
  ...
  //////////////////////////////////////////////////////////////////////////////////////
  // Output manager
  Teuchos::RCP<BasicOutputManager<ScalarType> > printer
    = Teuchos::rcp( new BasicOutputManager<ScalarType>(_verbosity) );
  ...
  if (globalTest_ == Teuchos::null) {
    convtest = Teuchos::rcp(new StatusTestResNorm<ScalarType,MV,OP>
			    (_convtol,nev,RITZRES_2NORM,_relconvtol) );
  }
  else {
  ...
  if (_ortho=="SVQB") {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(_ortho!="SVQB"&&_ortho!="DGKS",std::logic_error,"Anasazi::recycle::solve(): Invalid orthogonalization type.");
  }
...
*/
          ...
          if (_inSituRestart) {
            ...
            TEUCHOS_TEST_FOR_EXCEPTION(info != 0,std::logic_error,
                               "Anasazi::recycle::solve(): error calling GEQRF during restarting.");
        ...
        ////////////////////////////////////////////////////////////////////////////////////
          ...
          TEUCHOS_TEST_FOR_EXCEPTION(true,std::logic_error,"Anasazi::recycle::solve(): Invalid return from bks_solver::iterate().");
        }
      }
      catch (const AnasaziError &err) {
        printer->stream(Errors)
          << "Anasazi::recycle::solve() caught unexpected exception from Anasazi::BlockKrylovSchur::iterate() at iteration " << bks_solver->getNumIters() << std::endl
          << err.what() << std::endl
          << "Anasazi::recycle::solve() returning Unconverged with no solutions." << std::endl;
        return Unconverged;
      }
    }
  ...
  if (sol.numVecs < nev) {
    return Unconverged; // return from recycle::solve()
  }
  return Converged; // return from recycle::solve()
}


template <class ScalarType, class MV, class OP>
void
recycle<ScalarType,MV,OP>::setGlobalStatusTest(
...
recycle<ScalarType,MV,OP>::getGlobalStatusTest() const
{
  return globalTest_;
}
...
template <class ScalarType, class MV, class OP>
void
recycle<ScalarType,MV,OP>::setDebugStatusTest(
...
template <class ScalarType, class MV, class OP>
const Teuchos::RCP< StatusTest<ScalarType,MV,OP> > &
recycle<ScalarType,MV,OP>::getDebugStatusTest() const
{
  return debugTest_;
}
