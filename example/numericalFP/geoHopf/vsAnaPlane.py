...
# Vector field
def hopf(x, mu, gam, beta, delta):
    f = np.empty((2,))
    theta = np.angle(x[0] + 1j * x[1])
    mu = mu * (1 + delta * np.sin(theta))
    f[0] = x[0] * (mu - (x[0]**2 + x[1]**2)) \
           - x[1] * (gam - beta*(x[0]**2 + x[1]**2))
    f[1] = x[0] * (gam - beta*(x[0]**2 + x[1]**2)) \
           + x[1] * (mu - (x[0]**2 + x[1]**2))
    return f
...
for k in np.arange(muRng.shape[0]):
    ...
    plotDir = '%s/results/plot/numericalFP/%s' % (rootDir, model)
    ...
    Field = np.empty((nx0, nx0))
    R = np.abs(X + 1j* Y)
    PHI = np.angle(X + 1j* Y)
    omega = gam
    eta = R * (1. + delta * mu / (mu**2 + omega**2) * (omega * np.cos(PHI) + mu * np.sin(PHI) - omega * np.exp(mu / omega * PHI)))
    ...
    contourField = ax.contour(X, Y, eta,
                              20, linestyles='--', colors='k',
                              linewidths=2)
    # Set contour labels
