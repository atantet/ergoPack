...
saveEigVecForward = bool(int(sys.argv[9]))
saveEigVecBackward = bool(int(sys.argv[10]))
dim = 2
...
Tp = 2*np.pi/omega
dTp = - Tp * (1 + beta**2) / mu
wana = -eps**2 * (1 + beta**2) / (2*mu) + 1j*omega
wGaspard = -eps**2/2 * np.abs(dTp)/Tp * omega**2 + 1j*omega
print 'wAna = ', wana
