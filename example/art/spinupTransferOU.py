...
fig.savefig('plot/spinupTransferThesis0.%s' % ergoPlot.figFormat, bbox_inches='tight', dpi=ergoPlot.dpi)
...
ax.contour(X, Y, f, nlev, linewidths=2, colors='k', linestyles='--', label=r'$f(x)$')
ax.text(-0.1, -2.5, r'$\rho_0(x)$', fontsize=ergoPlot.fs_latex)
ax.contour(X, Y, ftr, nlev, linewidths=2, colors='k', label=r'$\mathcal{P}_t g(x)$')
ax.text(1.05, -0.8, r'$\mathcal{P}_t \rho_0(x)$', fontsize=ergoPlot.fs_latex)
...
fig.savefig('plot/spinupTransferThesis.%s' % ergoPlot.figFormat, bbox_inches='tight', dpi=ergoPlot.dpi)

fig = plt.figure()
ax = fig.add_subplot(111)
ax.contourf(X, Y, gtr, np.linspace(gtr.min(), gtr.max(), nlev+2)[1:], cmap=cm.Blues)
ax.text(-2.85, 0.75, r'$g(S(t) x)$', fontsize=ergoPlot.fs_latex)
...
#                             connectionstyle="arc3,rad=-0.1"))
#ax.text(xf3[0]-0.65, xf3[1]+0.2, r'$\phi(t) x_0^{(3)}$', color=simCol, fontsize=ergoPlot.fs_latex)
fig.savefig('plot/spinupKoopmanThesis.%s' % ergoPlot.figFormat, bbox_inches='tight', dpi=ergoPlot.dpi)
