...
# def hopf(x, mu, omega, beta):
#     f = np.empty((2,))
#     f[0] = x[0] * (mu - (x[0]**2 + x[1]**2)) \
#            - x[1] * (omega - beta*(x[0]**2 + x[1]**2))
#     f[1] = x[0] * (omega - beta*(x[0]**2 + x[1]**2)) \
#            + x[1] * (mu - (x[0]**2 + x[1]**2))
#     return f
...
# #muRng = np.array([-5.])
# muRng = np.arange(-4., 0., 1.)
# #muRng = np.arange(0., 5., 1.)
# #muRng = np.arange(5., 10., 1.)
# #muRng = np.array([0.])
# #muRng = np.array([3.])
# #muRng = np.array([7.])
# mu0 = -10.
...
#     else:
#         signBeta = 'p'
#     postfix = '_adapt_nx%d_k%03d_mu%s%02d_beta%s%02d_q%03d' \
#               % (nx0, k0 + k, signMu, int(round(np.abs(mu) * 10)),
#                  signBeta, int(round(np.abs(beta) * 10)), int(round(q * 100)))
...
#     print 'wAna = ', -q**2 * (1 + beta**2) / (2*mu) + 1j*(omega - beta*mu)
    
#     if not readEigVal:
#         # Define drift
#         def drift(x):
#             return hopf(x, mu, omega, beta)
...
#         np.savetxt('../results/numericalFP/ev_hopf%s.txt' % postfix, w)
