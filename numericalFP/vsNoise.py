...
eigVal2 = np.empty((qRng.shape[0], muRng.shape[0]))
...
for iq in np.arange(qRng.shape[0]):
    ...
    for k in np.arange(muRng.shape[0]):
        ...
        eigVal2[iq, k] = eigVal[1].real

    if q in qRngPlot:
        ax.plot(muRng, eigVal2[iq, :], linestyle='-', color=colors[cit],
...
fig.savefig('../results/plot/numericalFP/w2VSmuVSeps_hopf_adapt%s.%s' \
...
fig.savefig('../results/plot/numericalFP/w2VSeps_hopf_adapt%s.%s' \
            % (postfix, ergoPlot.figFormat), bbox_inches=ergoPlot.bbox_inches,
            dpi=ergoPlot.dpi)
